...
{{< img src="rc522-full.jpg" alt="Image" width="60.0%" class="center" >}}
...
- If you have a module as shown in the image above, it can only be used in SPI mode
  ([unless hacked](https://forum.arduino.cc/index.php?topic=442750.0)) and you must define a [SPI bus](#spi)
  in your configuration with both the **miso_pin** and **mosi_pin** set.

- If you have a RC522 which communicates via I²C (as on the M5 Stack), you need to have an [I²C](#i2c) bus configured.

## Over SPI

The `rc522_spi` component allows you to use RC522 NFC/RFID controllers with ESPHome. This component is a global hub
that establishes the connection to the RC522 via [SPI](#spi). Using [RC522 binary sensors](#rc522-binary_sensor),
...
- **cs_pin** (**Required**, [Pin Schema](#config-pin_schema)): The pin on the ESP that the chip select line
  is connected to.
- **reset_pin** (*Optional*, [Pin Schema](#config-pin_schema)): The pin connected to the RST line. Some tests
  shows the RC522 working okay without this.
- **update_interval** (*Optional*, [Time](#config-time)): The duration of each scan on the RC522. This affects the
...
that establishes the connection to the RC522 via [I²C](#i2c) (also available over SPI). Using the
