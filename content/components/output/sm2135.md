...
-  **data_pin** (**Required**, [Pin Schema](#config-pin_schema)): The pin used for MOSI.
-  **clock_pin** (**Required**, [Pin Schema](#config-pin_schema)): The pin which SCLK is
   connected to.
...
- **sm2135_id** (*Optional*, [ID](#config-id)): Manually specify the ID of the
...
at the same time. Therefore, the [Color Interlock](#rgbw_color_interlock) should be set to true
