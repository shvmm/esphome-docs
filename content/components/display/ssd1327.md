...
{{< img src="ssd1327-full.jpg" alt="Image" caption="SSD1327 OLED Display" width="75.0%" class="center" >}}

Connect CLK to the SCL (clock) pin you chose for the [I²C Bus](#i2c) and connect DIN to the SDA (data) pin.
...
- **address** (*Optional*, int): Manually specify the [I²C](#i2c) address of the display. Defaults to 0x3D.
...
Connect CLK to the SCK (clock) pin you chose for the [SPI bus](#spi), connect DIN to the MOSI/SDO pin, and connect `DC`
...
please see [color](#config-color) for more details. As this is a grayscale display, it only uses the white color
