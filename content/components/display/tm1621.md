...
{{< img src="tm1621-full.jpg" alt="Image" caption="TM1621 LCD Display." width="75.0%" class="center" >}}
...
- **cs_pin** (**Required**, [Pin Schema](#config-pin_schema)): The pin you have the CS line.
- **data_pin** (**Required**, [Pin Schema](#config-pin_schema)): The pin you have the DATA line.
- **read_pin** (**Required**, [Pin Schema](#config-pin_schema)): The pin you have the READ line.
- **write_pin** (**Required**, [Pin Schema](#config-pin_schema)): The pin you have the WRITE line.
- **lambda** (*Optional*, [lambda](#config-lambda)): The lambda to use for rendering the content on the TM1621.
...
The TM1621 has a similar API to the fully fledged [Display Rendering Engine](#display-engine), but it's only a subset as the TM1621
...
```
Please see [Formatted Text](#display-printf) for a quick introduction into the `printf` formatting rules.
