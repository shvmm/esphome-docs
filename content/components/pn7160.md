...
establishes a connection to the PN7160 via [SPI](#spi) or [I²C](#i2c).

{{< img src="pn716x-full.jpg" alt="Image" width="70.0%" class="center" >}}
...
[SPI bus](#spi) or the [I²C bus](#i2c).
...
The `pn7160_spi` component allows you to use [SPI-equipped](#spi) PN7160 NFC controllers with with ESPHome.
...
- **cs_pin** (**Required**, [Pin Schema](#config-pin_schema)): The pin connected to the PN7160's `NSS` (chip
  select) line.
- **dwl_req_pin** (*Optional*, [Pin Schema](#config-pin_schema)): The pin connected to the PN7160's
  `DWL_REQ` line. Used to invoke the PN7160's firmware update mode; may be used in a future release.
- **irq_pin** (**Required**, [Pin Schema](#config-pin_schema)): The pin connected to the PN7160's `IRQ` line.
- **ven_pin** (**Required**, [Pin Schema](#config-pin_schema)): The pin connected to the PN7160's `VEN` line.
- **wkup_req_pin** (*Optional*, [Pin Schema](#config-pin_schema)): The pin connected to the PN7160's
...
The `pn7160_i2c` component allows you to use [I²C-equipped](#i2c) PN7160 NFC controllers with ESPHome.
