...
This :ref:`action <config-action>` runs the ``sx127x`` image calibration, must be in standby mode.
...
This :ref:`action <config-action>` sets the ``sx127x`` mode to sleep.
...
:doc:`remote_receiver </components/remote_receiver>` should be used to receive
...
:doc:`remote_transmitter </components/remote_transmitter>` should be used to transmit
...
- :doc:`index`
- :doc:`/components/packet_transport/sx127x`
