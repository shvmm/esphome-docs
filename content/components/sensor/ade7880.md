...
Communication with the chip is over an [I2C bus](#i2c), so
...
- **irq1_pin** (**Required**, [Pin Schema](#config-pin_schema)):
...
- **irq0_pin** (*Optional*, [Pin Schema](#config-pin_schema)):
...
- **reset_pin** (*Optional*, [Pin Schema](#config-pin_schema)):
...
- **phase_a** (*Optional*): The configuration variables for the 'A'
  ...
  - **apparent_power** (instantaneous) (*Optional*): Report the
    ...
    from [Sensor](#config-sensor) are supported.
  ...
  - **forward_active_energy** (accumulated) (*Optional*): Report the
    ...
    detailed configuration mode, all options from [Sensor](#config-sensor) are supported.
...
- **i2c_id** (*Optional*, [ID](#config-id)): Specify the ID of the
  [I2C Component](#i2c) if your configuration includes multiple
  I2C buses.
