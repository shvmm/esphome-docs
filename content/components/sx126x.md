...
{{< img src="sx126x-full.png" alt="Image" width="50.0%" class="center" >}}
...
- **busy_pin** (**Required**, [Pin Schema](#config-pin_schema)): Busy pin.
- **cs_pin** (**Required**, [Pin Schema](#config-pin_schema)): SPI chip select pin.
- **dio1_pin** (**Optional**, [Pin Schema](#config-pin_schema)): Digital IO pin 1.
...
- **tcxo_delay** (**Optional**, [Time](#config-time)): The time needed for the TCXO to stabilize.
...
This [action](#config-action) runs the `sx126x` image calibration.
...
This [action](#config-action) sets the `sx126x` mode to tx.
...
This [action](#config-action) sets the `sx126x` mode to rx.
...
This [action](#config-action) sets the `sx126x` mode to sleep.
...
This [action](#config-action) sets the `sx126x` mode to standby.
...
This [action](#config-action) sends a packet.
