...
(`datasheet <https://www.semtech.com/products/wireless-rf/lora-connect/sx1262#documentation>`__) in
...
component may be used as a platform for the :ref:`packet-transport` component, enabling sensor data
to be sent directly from one ESPHome node to another.

.. figure:: images/sx126x-full.png
    :align: center
    :width: 50.0%
...
- **busy_pin** (**Required**, :ref:`Pin Schema <config-pin_schema>`): Busy pin.
- **cs_pin** (**Required**, :ref:`Pin Schema <config-pin_schema>`): SPI chip select pin.
- **dio1_pin** (**Optional**, :ref:`Pin Schema <config-pin_schema>`): Digital IO pin 1.
...
- **tcxo_delay** (**Optional**, :ref:`config-time`): The time needed for the TCXO to stabilize.
...
.. note::
    ...
    after calling ``configure``. See :apiref:`sx126x/sx126x.h`.

Automations:
------------

- **on_packet** (*Optional*, :ref:`Automation <automation>`): An automation to perform when a packet has
...
This :ref:`action <config-action>` runs the ``sx126x`` image calibration.
...
This :ref:`action <config-action>` sets the ``sx126x`` mode to tx.
...
This :ref:`action <config-action>` sets the ``sx126x`` mode to rx.
...
This :ref:`action <config-action>` sets the ``sx126x`` mode to sleep.
...
This :ref:`action <config-action>` sets the ``sx126x`` mode to standby.
...
This :ref:`action <config-action>` sends a packet.
