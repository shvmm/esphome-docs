...
Wow, what a week! As you may know, I wrote a blog post on the [Home Assistant web site](https://www.home-assistant.io/blog/2018/06/05/esphomelib/)
...
- esphomelib has finally gained support for a {{< docref "/components/status_led" "status LED" >}}. Now
  you can quickly identify if everything is OK with your ESP with a quick look at this LED. Happy raw:: html
...
- There are two new triggers available now: [esphomeyaml.on_boot](#esphome-on_boot) and
  [esphomeyaml.on_shutdown](#esphome-on_shutdown) with which you can do some advanced cleanup/setup
...
- All Hass.io add-on builds have now been more or less completely automated using a private Gitlab server. You
  can find all the new docker files [here](https://github.com/OttoWinter/esphomeyaml/tree/master/docker).
...
- The `inverted` option of binary sensors has been moved into the `filters:` section. So instead of
  ...
  ```
- The `esp32_ble` component and platform have been renamed to {{< docref "/components/esp32_ble_tracker" "esp32_ble_tracker" >}} in order to make the naming clearer with the new :doc:`esp32_ble_beacon
...
- The `ir_transmitter` component has been renamed to {{< docref "/components/remote_transmitter" "remote_transmitter" >}}
  as it now works with all kinds of protocols, not just infrared-based ones.

- The `pull_mode` option of the {{< docref "/components/sensor/pulse_counter" "Pulse Counter" >}} has been removed, please
  use the [Pin Schema](#config-pin_schema) now instead. Additionally, the `internal_filter` option now only accepts
  [time](#config-time) units.
